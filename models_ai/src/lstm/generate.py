"""
로또 번호 생성 스크립트 (LSTM 모델, 멀티 로또 지원)
...
from models_ai.src.lstm.lotto_lstm import create_model, LottoLSTM
...
def load_model(lottery_id: str) -> LottoLSTM:
    """저장된 모델 로드"""
    model_path = PROJECT_ROOT / f"models_ai/trained/lstm/{lottery_id}.pt"
    
    # Fallback to default model
    if not model_path.exists():
        model_path = PROJECT_ROOT / "models_ai/trained/lstm/lotto_model.pt"
...
def main():
    parser = argparse.ArgumentParser(description="AI 로또 번호 생성기 (LSTM)")
    ...
    try:
        ...
        print(f"\n❌ {e}")
        print("   먼저 실행: python scripts/train.py --model lstm --lottery " + lottery_id)
        return
